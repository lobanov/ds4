...
shapes, target quant types) + KV metadata. The actual quantized bytes are then
regenerated by `deepseek4-quantize` from the Hugging Face safetensors shards
46-48 of deepseek-ai/DeepSeek-V4-Flash-DSpark. See DSPARK.md for the full
...
Quant recipe (mirrors the existing MTP GGUF):
  F32   - norms, hc_*, attn_sinks, exp_probs_b
  F16   - hc_*_fn, ffn_gate_inp, hc_head_fn  (ds4 matmul_f16 kernels read these
          as F16; storing F32 caused the ffn_gate_inp routing bug — see issue468/27)
  ...
  BF16  - markov_head, confidence_head  (accuracy-critical)
  F32   - main_norm, output norm  (rmsnorm Metal kernel is F32-only)

NOTE: GGUF tensor type values follow this repo's ds4q_type enum (quants.h),
...
# ds4q_type enum values (quants.h) — NOT standard GGUF ids.
...
def tensors():
    ...
    for L in EMIT_LAYERS:
        ...
        add(f"{P}.hc_attn_fn.weight",    [HCM, HCD],            F16)
        add(f"{P}.hc_attn_scale.weight", [3],                   F32)
        add(f"{P}.hc_ffn_base.weight",   [HCM],                 F32)
        add(f"{P}.hc_ffn_fn.weight",     [HCM, HCD],            F16)
...
def main():
    import argparse
    ap = argparse.ArgumentParser(description="Write the DSpark drafter template GGUF (metadata only).")
    ap.add_argument("out", nargs="?", default="gguf/dspark_template.gguf",
                    help="output template path (default: gguf/dspark_template.gguf)")
