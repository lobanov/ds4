...
def main():
    ...
    if args.validate:
        cap = "issue468/baseline/dspark_capture"
        mh_p = np.load(f"{cap}/main_hidden_pos152.npy")
        mh_d = np.load(f"{cap}/main_hidden_pos153.npy")
        # anchor tokens: use plausible token ids (the oracle+port share them; exact
        # value only shifts which embed row is read). Use a common token + newline.
        anchor_p, anchor_d = 366, 366   # placeholder; refined below
    else:
    ...
    if args.validate:
        np.savez(f"{cap}/oracle_ref.npz", output_ids=out, logits=logits)
        print(f"  wrote {cap}/oracle_ref.npz")
