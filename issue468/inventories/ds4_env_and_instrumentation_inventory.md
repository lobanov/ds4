...
| `DS4_LEAD08_EXACT_SMALLM_PHASE` | diagnostic | Selects the V15 phase: `pair` runs Stage A; `direct` runs B1 M=2..8/C0-C4; `c5` runs position-103 replay/seam; `timing` runs the B2b fixed-work carrier. Other values fail closed. |
...
| `DS4_LEAD08_EXACT_SMALLM_TIMING_CSV` | diagnostic | Required by phase `timing`; records raw Metal timestamps/status and host intervals for the fixed-work B2b carrier. |
| `DS4_LEAD08_EXACT_SMALLM_TIMING_METADATA` | diagnostic | Required by phase `timing`; records all layer/site shapes, mapped offsets/bytes, APIs, and exact-kernel grid geometry. |
| `DS4_LEAD08_EXACT_SMALLM_TIMING_BLOCKS` | diagnostic | B2b measured-block count; only predeclared values 40 and the one allowed 80-block repeat are accepted. |
...
| `DS4_LEAD08_BATCH_M1_TARGET` | diagnostic | Routes normal Metal target M=1 evaluation and DSpark replay fallbacks through `metal_graph_verify_suffix_tops(..., n_tokens=1)`, sharing batched layer/output-head functions with M=K and using the batch-hidden push. Benchmark cycle JSON reports batched/raw M1 path counts. Retained to reproduce iteration 10's shared-family NO-GO; rejects MTP and is not an optimization. |
