...
| V15 | Exact-row small-M dense family capability gate | Could fit the 43 ms conservative policy base while removing exposed dense divergence | Stage A is bit-exact against two independent production M1 calls | Q8+F16 at every stage; one traversal; nonredundant output-B seam | Stage B M2..8/seam/cumulative timing after the passed M2 spike | Stage A: 430/430 Q8 Q-a/F16 router cases exact; AIR logical traversal passes; no speed claim | `P0` Stage B GO; research-only |
...
- V13-V15: `34_iter26_exact_smallm_dense_inventory.md`,
  `35_iter27_exact_smallm_protocol.md`, and `36_iter28_exact_smallm_pair.md`; Stage A passes while
  Stage B remains the P0 economic/capability gate.
...
| P0 | V15 Stage B full family/economics | Admitted by Stage A pass | Both formats M2..8, seven sites/all offsets, C0-C4 for M2..8, C5 for M2..4, nonredundant B, M4 all-43 delta upper <=7.5 ms | One validity repeat only; otherwise stop |
