...
| V15 | Exact-row small-M dense family capability gate | Could fit the 43 ms conservative policy base while removing exposed dense divergence | Stage A/B1 and captured B2a matrices are bit-exact against M independent production M1 calls | Q8+F16 at every stage; one traversal; nonredundant output-B seam | 40-block timing plus one 80-block validity repeat | Nominal C-minus-E ~-3.47 ms, but both runs fail stratified stability; no speed claim | `CLOSED STOP`; persistent timing ambiguity |
...
- V13-V15: `34_iter26_exact_smallm_dense_inventory.md`,
  ...
  `37_iter29_exact_smallm_direct.md`, `38_iter30_c5_seam.md`, and
  `39_iter31_fixed_work_timing.md`; correctness/seam pass, but B2b closes on persistent ambiguity.
...
| Closed | V15 Stage B2b dense-carrier economics | Final Stage-B dependency | Nominal upper -3.442/-3.456 ms, but 40/80-block stratified stability fails | Persistent ambiguity is STOP; no graph rollout |
